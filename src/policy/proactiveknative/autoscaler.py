...
from src.models.sgd import predict_min_containers
...
class ProactiveKnativeAutoscaler(Autoscaler):
    ...
    def scaling_level(self, system_state: ProactiveKnativeSystemState, task_type: TaskType):
        ...
        # no_replicas = self.models[task_type['name']].predict(np.array([events]).reshape(1, -1))[0]
        no_replicas = predict_min_containers(events, self.models[task_type['name']])
