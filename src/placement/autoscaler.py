...
class Autoscaler:
    ...
    def log_system_status(self, replicas: Dict[str, Set[Tuple[Node, Platform]]]):
        for function_name, function_replicas in replicas.items():
            ...
            queue_lengths_by_platform = defaultdict(list)
            available_memory_by_node = defaultdict(list)
            available_platforms_by_node = defaultdict(list)

            for node, platform in function_replicas:
                count_by_platform_type[platform.type['shortName']] += 1
                queue_lengths_by_platform[platform.type['shortName']].append(len(platform.queue.items))
                available_memory_by_node[node.node_type].append(node.available_memory)
                available_platforms_by_node[node.node_type].append(node.available_platforms)
            ...
            event: SystemEvent = {
                ...
                "queue_lengths_by_platform": queue_lengths_by_platform,
                "available_memory_by_node": available_memory_by_node,
                "available_platforms_by_node": available_platforms_by_node
            }
