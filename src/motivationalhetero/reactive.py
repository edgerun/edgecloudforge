...
from src.train import save_models, \
    train_dtr_model_reactive_then_proactive_per_device_type
...
def main():
    ...
    models = train_dtr_model_reactive_then_proactive_per_device_type(result_files, include_queue_length=False, test_size=test_size, until=one_day_until, encoder=encoder)
    ...
    # first week training
    result_files = [f'{x}/peak-config.json' for x in result_folders[:1]]
    models = train_dtr_model_reactive_then_proactive_per_device_type(result_files, include_queue_length=False, test_size=test_size, encoder=encoder)
